...
typedef struct {
    ...
    /* NEW: store the exact REB intervals (one interval per intersection we covered).
       These intervals are printed in the REB file as separate lines for the role.
    */
    Interval *reb_intervals;
    int reb_intervals_count;
    int reb_intervals_cap;
} Role;
    ...
    while (1) {
        ...
        for (int u = 1; u <= ds->num_users; u++) {
            ...
            for (int k = 0; k < ucount; k++) {
                ...
                /* Build T as the intersection interval common to all selected perms for the seed user.
                   This makes the role time conservative and reduces mismatches with the original TUPA. */
                double maxStart = -1e308, minEnd = 1e308;
                for (int pp = 0; pp < candidate_p_ct; pp++) {
                    ...
                    for (int kk = 0; kk < ucount; kk++) {
                        ...
                        if (te->pid == pid && !te->covered) {
                            if (te->t.start > maxStart) maxStart = te->t.start;
                            if (te->t.end < minEnd) minEnd = te->t.end;
                            found = 1;
                            break;
                        }
                    }
                    if (!found) { maxStart = 1e308; minEnd = -1e308; break; }
                }
                /* require a non-empty intersection */
                if (maxStart >= minEnd - EPSILON) {
                    free(candidate_pids);
                    candidate_pids = NULL;
                    continue;
                }
                Interval candT = { maxStart, minEnd };
        ...
        /* --- NEW: capture exact intersection intervals for REB output --- */
        newr.reb_intervals = NULL;
        newr.reb_intervals_count = 0;
        newr.reb_intervals_cap = 0;

        for (int a = 0; a < newr.temp_num_covered; ++a) {
            int ei = newr.temp_covered_indices[a];
            if (ei >= ds->count) continue;
            TupaEntry *ent = &ds->entries[ei];
            /* compute the exact intersection between ent->t and best_T */
            double inter_s = max_d(ent->t.start, newr.t.start);
            double inter_e = min_d(ent->t.end, newr.t.end);
            if (inter_e > inter_s + EPSILON) {
                Interval iv = { inter_s, inter_e };
                if (newr.reb_intervals_count >= newr.reb_intervals_cap) {
                    newr.reb_intervals_cap = (newr.reb_intervals_cap == 0) ? 8 : newr.reb_intervals_cap * 2;
                    newr.reb_intervals = realloc(newr.reb_intervals, newr.reb_intervals_cap * sizeof(Interval));
                    if (!newr.reb_intervals) { perror("realloc reb_intervals"); exit(1); }
                }
                newr.reb_intervals[newr.reb_intervals_count++] = iv;
            }
        }
...
// Fix 5: Reconciliation step
// Ensure every original UPA+TIME triple is represented by some role's UA+PA+REB.
// If not, create a tiny role that covers exactly that triple.
void reconcile_with_original_upa(const char *upa_path, const char *time_path, Dataset *ds, Role **roles_ptr, int *num_roles_ptr) {
    Role *roles = *roles_ptr;
    int num_roles = *num_roles_ptr;
    int roles_cap = (num_roles > 0) ? num_roles * 2 : 1024;

    // Helper: ensure roles array capacity
    auto_ensure_capacity:
    if (num_roles >= roles_cap) {
        roles_cap *= 2;
        roles = realloc(roles, roles_cap * sizeof(Role));
        if (!roles) { perror("realloc roles (reconcile)"); exit(1); }
    }

    // Open original files
    char upa_full[MAX_PATH], time_full[MAX_PATH];
    sprintf(upa_full, "%s%s", BASE_DIR, upa_path);
    sprintf(time_full, "%s%s", BASE_DIR, time_path);

    FILE *f_upa = fopen(upa_full, "r");
    FILE *f_time = fopen(time_full, "r");
    if (!f_upa || !f_time) { 
        fprintf(stderr, "Reconcile: can't open original files %s %s\n", upa_full, time_full);
        if(f_upa) fclose(f_upa);
        if(f_time) fclose(f_time);
        *roles_ptr = roles; *num_roles_ptr = num_roles;
        return;
    }

    char line_upa[MAX_LINE], line_time[MAX_LINE];
    int orig_num_users = 0, orig_num_perms = 0;
    // read headers (first three lines as in load_data)
    if (fgets(line_upa, MAX_LINE, f_upa)) orig_num_users = atoi(line_upa);
    if (fgets(line_upa, MAX_LINE, f_upa)) orig_num_perms = atoi(line_upa);
    if (fgets(line_upa, MAX_LINE, f_upa)) { /* skip count line */ }

    // iterate every original triple line-by-line
    while (fgets(line_upa, MAX_LINE, f_upa) && fgets(line_time, MAX_LINE, f_time)) {
        int uid, pid;
        if (sscanf(line_upa, "%d %d", &uid, &pid) != 2) continue;
        // parse TIME line: may contain multiple pairs start end
        char *ptr = line_time;
        double s,e;
        int off;
        while (sscanf(ptr, "%lf %lf%n", &s, &e, &off) == 2) {
            Interval orig_iv = { s, e };

            // Check if some existing role covers (uid,pid,orig_iv)
            int covered_by_some_role = 0;
            for (int r = 0; r < num_roles && !covered_by_some_role; ++r) {
                if (!roles[r].valid) continue;
                // role must contain uid and pid
                int has_u = 0, has_p = 0;
                for (int uu = 0; uu < roles[r].num_uids; ++uu) if (roles[r].uids[uu] == uid) { has_u = 1; break; }
                if (!has_u) continue;
                for (int pp = 0; pp < roles[r].num_pids; ++pp) if (roles[r].pids[pp] == pid) { has_p = 1; break; }
                if (!has_p) continue;

                // role must have at least one REB interval that covers the orig_iv (intersection >= full orig_iv)
                int ri_count = roles[r].reb_intervals_count;
                for (int ri = 0; ri < ri_count; ++ri) {
                    Interval riv = roles[r].reb_intervals[ri];
                    // If the role interval covers orig interval (or sufficiently overlaps)
                    double inter_s = max_d(riv.start, orig_iv.start);
                    double inter_e = min_d(riv.end, orig_iv.end);
                    if (inter_e > inter_s + EPSILON) {
                        // If intersection covers entire orig_iv within EPSILON, treat as covered
                        if ((orig_iv.end - orig_iv.start) <= EPSILON || (inter_e - inter_s) >= (orig_iv.end - orig_iv.start) - 1e-6) {
                            covered_by_some_role = 1;
                            break;
                        }
                    }
                }
            ...
            if (!covered_by_some_role) {
                // Need to add a tiny role for exactly (uid, pid, orig_iv)
                if (num_roles >= roles_cap) goto auto_ensure_capacity;

                Role nr;
                nr.uids = NULL; nr.num_uids = 0; nr.uids_capacity = 0;
                nr.pids = NULL; nr.num_pids = 0; nr.pids_capacity = 0;
                nr.t.start = orig_iv.start; nr.t.end = orig_iv.end;
                nr.temp_covered_indices = NULL; nr.temp_num_covered = 0;
                nr.valid = 1; nr.split_count = 0; nr.overlap_count = 0;
                nr.id = num_roles + 1;

                // UA: single user
                add_unique(&nr.uids, &nr.num_uids, &nr.uids_capacity, uid);
                // PA: single permission
                add_unique(&nr.pids, &nr.num_pids, &nr.pids_capacity, pid);
                // REB intervals: single exact interval
                nr.reb_intervals = NULL; nr.reb_intervals_count = 0; nr.reb_intervals_cap = 0;
                add_to_array((int**)&nr.reb_intervals, &nr.reb_intervals_count, (int*)&nr.reb_intervals_cap, 0); // placeholder allocation hack
                // cleanup the placeholder and set proper interval
                free(nr.reb_intervals);
                nr.reb_intervals_cap = 1;
                nr.reb_intervals = malloc(sizeof(Interval) * nr.reb_intervals_cap);
                if (!nr.reb_intervals) { perror("malloc nr.reb_intervals"); exit(1); }
                nr.reb_intervals[0] = orig_iv;
                nr.reb_intervals_count = 1;
...
void write_outputs(Dataset *ds, Role *roles, int num_roles) {
    ...
    /* For PA file: count total role-permission lines first */
    int total_pa_lines = 0;
    for (int i = 0; i < num_roles; ++i) {
        if (!roles[i].valid) continue;
        total_pa_lines += roles[i].num_pids;
    }
    ...
    for(int i=0; i<num_roles; i++) {
        ...
        /* print each stored REB interval for this role (one per line) */
        if (roles[i].reb_intervals_count > 0) {
            for (int ri = 0; ri < roles[i].reb_intervals_count; ++ri) {
                Interval iv = roles[i].reb_intervals[ri];
                fprintf(reb, "r%d %.1f %.1f\n", current_id, iv.start, iv.end);
            }
        } else {
            /* fallback: if none stored, print the role's t as before */
...
int main(int argc, char *argv[]) {
    ...
    // Reconciliation: ensure all original triples are covered exactly
    reconcile_with_original_upa(INPUT_UPA, INPUT_TIME, &ds, &roles, &num_roles);
    ...
    if (roles[i].reb_intervals) free(roles[i].reb_intervals);
    }
    free(roles);
    return 0;
}
