...
void solve_cotrapmp(Dataset *ds, double theta, Role **roles_out, int *num_roles_out) {
    ...
    printf("\nStarting Role Mining (Algorithm 2 - MinSplit First)...\n");
    ...
    while (found_any) {
        ...
        for (int i = 0; i < initial_count; i++) {
            ...
            // *** SELECTION HEURISTIC CHANGED ***
            // Priority 1: MINIMIZE SPLITS (Fragmentation is the enemy of minimization in c100)
            // Priority 2: MAXIMIZE COVERAGE
            int is_better = 0;
            
            if (current_splits < min_splits) {
                is_better = 1;
            } else if (current_splits == min_splits) {
        ...
        if (found_pass && max_coverage > EPSILON) {
            ...
            // Handle dynamic growth of idx_map
            if (ds->capacity + 2000 > (ds->capacity + 5000)) { // Simple check
                // Realloc map check buffer if dataset grew too much
                free(idx_map);
